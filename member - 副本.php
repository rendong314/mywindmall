...
else if($a == 'address_save')
{
    ...
    if($sname == '' or $mobile == '' or $code == '' or $postarea_prov == '' or $postarea_city == '' or $postarea_country == '' or $address == ''){
    ...
    if(isset($action) && $action == 'add'){
    	...
    	$sql = "INSERT INTO `#@__address` (uid,sname,mobile,province,city,country,address,code) VALUES ('$uid','$sname','$mobile','$postarea_prov','$postarea_city','$postarea_country','$address','$code')";	
        ...
        $sql = "UPDATE `#@__address` SET sname='$sname', mobile='$mobile', province='$postarea_prov', city='$postarea_city',country='$postarea_country',address='$address',code='$code' WHERE id=$id";
        if($dosql->ExecNoneQuery($sql))
        {
...
if(isset($_GET['action']) && $_GET['action']=='ok'){
		...
		$options = array (
            'token' => '140fb9872da87d4e6d5559d571914584', // 填写你设定的key
            'encodingaeskey' => 'Mz85Ffl6PrqJp9PNY6RkAJRF5zznFej95Nj1IXxZQ3M', // 填写加密用的EncodingAESKey
            'appid' => 'wx971705e9d6437ad3', // 填写高级调用功能的app id
            'appsecret' => '518cd6da5bf23cc2f4a7e05628e671f2' // 填写高级调用功能的密钥
		    //         'partnerid' => '', // 财付通商户身份标识
...
else if($a == 'delfavorite')
{
	...
	if(is_array($checkid))
	{
		foreach($checkid as $v)
		{
			//参数过滤
			$v = intval($v);
			$dosql->ExecNoneQuery("DELETE FROM `#@__userfavorite` WHERE `id`=$v AND `uid`='$c_uname'");
		}
	}
