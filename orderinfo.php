...
$sel = isset($sel) ? $sel : '';
$buynum = isset($buynum) ? $buynum : '';
$c_uname = AuthCode($_COOKIE['username']);
//更新购物车表
for($i=0;$i<count($_POST["sel"]);$i++){
	$r = $dosql -> GetOne("SELECT * FROM `#@__goodscar` WHERE id = ".$_POST["sel"][$i]."");
	$shuliang=$_POST["buynum"][$r["goodsid"]];
	$dosql->execnonequery("update `#@__goodscar` set `count`='".$shuliang."' where uid='".$c_uname."' and id='".$r['id']."'");
}
...
<!--付款方式 END-->      
	...
	for($i=0;$i<count($_POST["sel"]);$i++){
		...
		ShowMsg("订单已提交","index.php ");
		}
		$r1 = $dosql -> GetOne("SELECT * FROM `#@__goods` WHERE id = ".$r['goodsid']."");
		$shuliang=$_POST["buynum"][$r["goodsid"]];	
		...
		if($r["attrid"]){
			$integral=$r_jiage['jifen']*$shuliang;
			$sumattr +=$integral;
			}else{
			$jifen=$r1['integral']*$shuliang;
			$sum +=$jifen;	
			}
		...
		$row = $dosql -> GetOne("SELECT * FROM `#@__goodscar` WHERE id = ".$_POST["sel"][$i]." and peisong='true'");			
		if($row){
			if($row['attrid']!='0'){
				...
				$peisong_jiage = $dosql -> GetOne("select * from `#@__goods` where id='".$r['goodsid']."'");
				$ps_jiage .= $peisong_jiage['salesprice']*$shuliang;
				$ps_jiage;	
				}
			}
...
?>
      <input type="hidden" value="<?php echo $r['id']; ?>" name="carid[]">
      <input type="hidden" value="<?php echo $r['attrid']; ?>" name="attrid[]">
      <input type="hidden" value="<?php echo $r1['id']; ?>" name="goodsid[]">
      <input type="hidden" value="<?php echo $shuliang ?>" name="goodsnum[]">
      ...
      <input type="hidden" value="<?php echo $sum+$sumattr;?>" name="jifen[]" >
...
<!--代金券 END-->         
      ...
      <div class="ddy_djq"><span class="fl">可得积分</span><font class="red fr"><?php echo $sum+$sumattr;?></font>
        <div class="cl"></div>
      </div>
      <script type="text/javascript">
function hyj()
{
    ...
    <input type="hidden" id="aa" name="aa" value="dingdanadd">
    <!--footer start-->
    <footer>
      <div style="height:60px;"></div>
      <ul class="pro_xqy_bot tc">
        <li class="fl" style="margin-left:2%; color:#F00;">需付：￥
          <input type="text" value="<?php if($shiah!='829'){if($ps_jiage > GetFragment(14)){echo $he;}else{echo $he+GetFragment(9);}}else{echo $he;}?>" name="fukuan" id="price1" style="width:55px; border:0px; background-color:#f7f7f7;color:red;color: red; -webkit-text-fill-color:red; -webkit-opacity:1; opacity: 1;  font-size:14px;" disabled>
        </li>
