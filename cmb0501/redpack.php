...
function SendRedpack($pay_openid,$msg_openid,$red_type,$redpack_money,$sendname,$wish,$actname,$gid){
	global $dosql;
	$toOpenId=$pay_openid;
	$redpack_price=$redpack_money;
	$hb = new WXHongBao();
	$hb->newhb($toOpenId,$redpack_price); 
	$hb->setNickName("有限公司");
	$hb->setSendName($sendname);
	$hb->setWishing($wish);
	$hb->setActName($actname);
	$hb->setRemark("任性一把");
		
	if(!$hb->send()){
		
		$dosql->ExecNoneQuery("insert into `cmb_redpack_0501`(pay_openid,redpack,red_status,red_type,gid,red_time) values('$pay_openid',$redpack_money,2,$red_type,$gid,".time().")");
		return "2";//发送失败
	}else{
		$dosql->ExecNoneQuery("insert into `cmb_redpack_0501`(pay_openid,redpack,red_status,red_type,gid,red_time) values('$pay_openid',$redpack_money,1,$red_type,$gid,".time().")");
		return "1";//发送成功
	}
...
CLASS WXHongBao {
  ...
  private $mch_id = "1435680502";//商户ID写死
  private $wxappid = "wxd0a329f2b1cff07b";//微信公众号，写死
  private $client_ip = "123.56.88.104"; //调用红包接口的主机的IP,服务端IP,写死，即脚本文件所在的IP
  private $apikey = "M78v277j9xAkbZlOmfEAwsaMVTqIPPqc";//pay的秘钥值
  ...
  function __construct(){
    ...
    $this->apiclient_cert = getcwd()."/cert1/apiclient_cert.pem";
    $this->apiclient_key = getcwd()."/cert1/apiclient_key.pem";
  ...
  public function send($url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/sendredpack",$total_num = 1){
    ...
    //debug
    file_put_contents("hbxml.txt",$xml,file_append);
