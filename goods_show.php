...
<div class="cart_fixed_bg">
  <div class="cart_fixed">
    <div class="cart_con">
      ...
      <input type="hidden" id="attrid" value="<?php echo $r['guige']; ?>">
...
<!--规格JS END-->      
      <li><span class="fl">购买数量</span><span class="fr cart_sum"> 
        ...
        <input type="text" name="buynum" value="1" class="fl tc cart_sum_inp" id="qty_item_1" onKeyUp="setAmount.modify('#qty_item_1')" readOnly="true">
...
<div class="banner mar_5">
  <div id="Banner">
	<div class="swiper-container">
		...
		?>
			<div class="swiper-slide"><a href="#"><img src="/<?php echo $goodspicarr?>" alt=""></a></div>
		<?php 
				  }}else{?>
					  <div class="swiper-slide"><a href="#"><img src="/<?php echo $r['picurl'];?>" alt=""></a></div>
				<?php  }
		?>
		</div>
...
<section>
  <div class="pro_xqy_con line_2"><span class="fl pro_xqy_tit"><?php echo $r['title'];?><br>
	...
	<!--<span class="fr pro_collection tc" onclick="AddUserFavorite(<?php echo $id; ?>,4)">
  ...
  <?php  $zan_img = $dosql->GetOne("SELECT id FROM `#@__zan` WHERE goodsid='$id' and uid='$c_uname'");?>
  <div class="pro_xqy_con"><span class="fl">优惠：<?php if($r['songhuo']=='true'){echo GetFragment(8);}else{ echo "暂无优惠";}?></span> <span class="fr">销量：<?php echo $r['monthsell'];?></span> 
<!--
  <span class="fr lqb_zan" style=" margin-right:5%;-webkit-tap-highlight-color:transparent;" onClick="zan(<?php echo $id;?>,<?php
	if(empty($zan_img)){ echo "1";}else{echo "2";}?>,<?php echo $c_uname;?>)">
   <?php $zan_img = $dosql->GetOne("SELECT id FROM `#@__zan` WHERE goodsid='$id' and uid='$c_uname'");
	if(empty($zan_img)){
	?>
   <img src="images/zan_hui.png" class="zan" style="width:18px; height:18px;">
    <?php
	}else{
	?>
   <img src="images/zan.png" class="zan" style="width:18px; height:18px;">
   <?php }?>  
  ：<span class="zanshu">
   <?php 
...
<!--点赞AJAX  
<script>
 function zan(goodsid,flag,uid){
	 $.ajax({
		url : "Ajax_dianzan.php",
		type: "post",
		async : false,
		data:{"flag":flag, "goodsid":goodsid, "uid":uid},
		dataType:'html',
		success:function(data){
		if(data==1){
				...
				$(".zanshu").text(parseInt($(".zanshu").text())+parseInt(1));
				$(".lqb_zan").attr("onclick","zan(<?php echo $id;?>,2,<?php echo $c_uname;?>)");
				return false;
				}else if(data==2){
				...
				$(".lqb_zan").attr("onclick","zan(<?php echo $id;?>,1,<?php echo $c_uname;?>)");
				$(".xianshi").text("取消成功！").show(300);
			 	setTimeout(function () {
				$(".xianshi").hide(300);
				}, 2000);
				return false;	
...
<!--
  <div class="pro_jf mar_3" style="-webkit-tap-highlight-color:transparent;"><span class="pro_jf_bor fl">积分</span><span class="fl" id="jf">
    <?php if($r['integral']!='0'){?>
    购买可获得<span class="xiaowei"><?php echo $r['integral'];?></span>积分
    <?php }else{?>
    此商品暂不支持积分！
    <?php }?>
 ...
 <div class="pro_xqy_text"> <?php echo $r['guige'];?> </div>
...
?>
		...
		?>
        <div style="height:40px; text-align:center; line-height:40px;">暂无相关推荐！</div>
        <?php }?>
        <div class="cl"></div>
      </ul>
      <div class="cl"></div>
...
<footer>
  ...
  <ul class="pro_xqy_bot tc">
    <li class="pro_xqy_bot_icon01 fl" style="margin-left:5%; background:#FFF;"><a href="goodscar.php"><img src="images/a13.png" alt=""></a></li>
    <li class="pro_xqy_bot_icon01 fl" style="margin-left:3%;background:#FFF;"><a href="member.php?c=default"><img src="images/a14.png" alt=""></a></li>
